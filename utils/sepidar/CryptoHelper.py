...
def rsa_encrypt(public_key: str, plain_text: bytes):
    ...
    modulus = int.from_bytes(b64decode(xml.find('Modulus').text.strip()), 'big')
    exponent = int.from_bytes(b64decode(xml.find('Exponent').text.strip()), 'big')
