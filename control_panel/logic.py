...
def refresh_code_stat_names(
    *, limit: Optional[int] = None, scope: Optional[str] = None
) -> Dict[str, Any]:
    scope_key = (scope or "missing").strip().lower()
    force_all = scope_key in {"all", "full", "everything", "force", "all-records"}
    ...
    try:
        updated = refresh_missing_code_names(limit=safe_limit, scope=scope_key)
    except Exception as exc:
    ...
    return {
        "updated": int(updated),
        "limit": safe_limit if safe_limit is not None else None,
        "scope": scope_key,
    }
